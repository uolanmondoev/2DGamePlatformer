...
class Player():
	...
	def update(self, game_over):
		...
		if game_over == 0:
			...
			draw_text("GAME OVER!", font, blue, (screen_width // 2) - 160, screen_height - 450 )
			if self.rect.y > 50:
				self.rect.y -= 5
		...
		for row in data:
			...
			for tile in row:
				...
				if tile == 2:
					...
					self.tile_list.append(tile)
				if tile == 3:
					zombie = Enemy(col_count * tile_size, row_count * tile_size - 5)
				...
				if tile == 7:
					...
					coin_group.add(coin)
				if tile == 8:
					exit = Exit(col_count * tile_size, row_count * tile_size - (tile_size // 2) + 5)
					exit_group.add(exit)
...
#create buttons
restart_button = Button(screen_width // 2 - 100, screen_height // 2, restart_image)
	...
	if main_menu == True:
		...
		if game_over == -1:
			if restart_button.draw():
				...
				pygame.mixer.Sound.stop(main_theme)
				music_on = False
				game_over = 0
				score = 0

		#if player has complited the level
		if game_over == 1:
			#reset game and fo to next level
			level += 1
			if level <= max_levels:
				#reset level
				world_data = []
				...
				draw_text('YOU WIN!', font, blue, (screen_width // 2) - 110, screen_height - 450)
				victory_fx.play()
				#restart game
				if restart_button.draw():
					level = 1
